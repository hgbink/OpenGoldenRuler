...
namespace HLGranite.WPF
{
    ...
    public class Ruler : FrameworkElement
    {
        ...
        private double _currentReading = 0;
        #endregion
        ...
        public double Length
        {
            get
            {
                if (this.AutoSize)
                {
                    return (double)(Unit == Unit.Cm ? DipHelper.DipToCm(this.ActualWidth) : (Unit == Unit.Inch ? DipHelper.DipToInch(this.ActualWidth) : this.ActualWidth)) / this.Zoom;
                }
                else
                {
        ...
        /// <summary>
        /// The value of current reading
        /// </summary>
        public double CurrentReading { set { _currentReading = value; } get { return _currentReading; } }
        ...
        /// It only support DIP at the moment.
        /// </summary>
        public Unit Unit
        {
        ...
        #region Methods
        /// <summary>
        /// Used to catch the current reading
        /// </summary>
        ...
        void Ruler_MouseMove(object sender, System.Windows.Input.MouseEventArgs e)
        {
            if (e != null)
            {
                ...
                if (CurrentReading == point.X) return;
                // if current reading changed, get ruler length and reading

                CurrentReading = point.X;
            }
        }
        ...
        protected override void OnRender(DrawingContext drawingContext)
        {
            ...
            Pen background = new Pen(Brushes.Lavender, 1){StartLineCap = PenLineCap.Round, EndLineCap = PenLineCap.Round};
            drawingContext.DrawRectangle(Brushes.Lavender, background, new Rect(new Point(0, 0), new Size(Length, Height)));

            double xDest = (Unit == Unit.Cm ? DipHelper.CmToDip(Length) : (Unit == Unit.Inch ? DipHelper.DipToInch(Length) : Length)) * this.Zoom;
            drawingContext.DrawRectangle(null, BorderPen, new Rect(new Point(0.0, 0.0), new Point(xDest, Height)));
            double chip = Unit == Unit.Cm ? DipHelper.CmToDip(Chip) : (Unit == Unit.Inch ? DipHelper.InchToDip(Chip) : Chip);
            drawingContext.DrawLine(RedPen, new Point(_currentReading, chip), new Point(_currentReading, Height));
            ...
            for (double dUnit = 0; dUnit <= Length; dUnit++)
            {
                ...
                {
                    d = 100 * (dUnit) * this.Zoom;

                    if (dUnit < Length)
                    {
                        if (Marks == MarksLocation.Up)
                        {
                            double length = dUnit * this.Zoom;
                            if (dUnit % 20 == 0)
                        ...
                        else
                        {
                            double length = dUnit * this.Zoom;
                            if (dUnit % 20 == 0)
                                drawingContext.DrawLine(ThinPen, new Point(length, Height), new Point(length, Height - SegmentHeight * 2D / 3D));
                            else if (dUnit % 10 == 0)
                                drawingContext.DrawLine(ThinPen, new Point(length, Height), new Point(length, Height - SegmentHeight / 3.0));
                        }
                    }
                }
                ...
                else if(d < Length)
                {
                    ...
                    if ((dUnit != 0.0) && (dUnit < Length))
                    {
                        ...
                        if (Marks == MarksLocation.Up)
                            drawingContext.DrawText(ft, new Point(d+10, Height));
                        else
                            drawingContext.DrawText(ft, new Point(d+10, Height - SegmentHeight - ft.Height));
                    }
                }
        ...
        protected override Size MeasureOverride(Size availableSize)
        {
            ...
            {
                desiredSize = new Size((Length), Height);
            }
    ...
    public enum Unit
    {
        ...
        /// <summary>
        /// The unit is DIP (Device Independent Pixels)
        /// </summary>
        Dip
    };
